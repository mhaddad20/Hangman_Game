...
$hash_hints={"MAESTRO"=>"Master of any arts","BIRTHDAY"=>"Blow out the candles","VEGETABLE"=>"Healthy produce","HALF"=>"50%","CACTUS"=>
  "contains water", "QUEEN"=>"Elizabeth","MAGIC"=>"Abracadabra","GOBLIN"=>"Halloween Costume",
            ...
            "INTERNET"=>"World wide web","VIBRANT"=>"Full of energy","OBEDIENT"=>"Teacher's pet"}
...
class Game
  ...
  def initialize
    ...
    @finished = false
    @word_loc = rand(15)
    @word = $hash_words[@word_loc]
    @word_hint = $hash_hints[@word]
    $arr=[]
    $chosen_letters=[]
    $correct_letters_arr=[]
  ...
  def text_guesses
    @guess_remain=Text.new("GUESSES REMAINING: #{@guesses}")
  end
  def hint
    Text.new("Hint: #{@word_hint}",x:Window.width/2,y:0)
  end
  def game_over_text
    if check_winner
  ...
  def change_opacity(shape)
    shape.opacity=0.4
  end

  def draw(x1,y1)
    background = Image.new("img3.jpg",y:30 )
    x=WIDTH_X
    y = HEIGHT_Y
    for a in 1..$array_alphabet.size
      @shape=Square.new(x:x,y:y,size:35,color:'orange')
      Text.new("#{$array_alphabet[a-1]}",x:x+(x/17),y:y,size:20,color:'white')
      if @shape.contains?(x1,y1)
        if @word.include?($array_alphabet[a-1])
    ...
    for x in 0..@word.length-1
      if($arr[x])
        Text.new(@word[x],x:HEIGHT_Y+(line_width*x),y:HEIGHT_Y*4,size:40)
      end
  ...
  def gallows
      ...
      if @guesses==0
        right_leg=Line.new(x1:600,y1:275,x2:625,y2:325,color:'black',size:200)
      end
  end
  def check_winner
      $arr.all?{|x| x==true}
...
update do
  if game.guesses==0 || game.check_winner
    game.game_finish
    game.game_over_text
  end
end
